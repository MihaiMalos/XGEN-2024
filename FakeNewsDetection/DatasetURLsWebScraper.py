class DatasetScraper:
    ...
    def extract_text_from_website1(self, article_soup):
        """
    ...
    def extract_text_from_website2(self, article_soup):
        """
    ...
    def extract_text_from_website3(self, article_soup):
        """
    ...
    def extract_text_from_website4(self, article_soup):
        """
    ...
    def extract_text_from_website5(self, article_soup):
        """
          Extract the text from the first two paragraphs with the specified class from the article.
    ...
    def extract_text_from_website6(self, article_soup):
        """
        ...
        if container:
            container = container.find('div',
                                       class_='main-content main-content--desktop-article main-content--universal-header')
        if container:
            container = container.find('div',
                                       class_='pay-wall-content content_664cb6592785155cfe42bdf8 current-page ads-loaded')
        if container:
        ...
        if container:
            container = container.find('div',
                                       class_='article-body fs-article fs-responsive-text current-article article-topline')

        # Find the first <p> element within the final container
        if container:
            paragraph_element = container.find('p')
            if paragraph_element:
                raw_text = paragraph_element.get_text(separator='\n').strip()
                return self.clean_text(raw_text)

        return None

    def extract_articles_from_page(self, max_words=200):
        ...
        if response.status_code == 200:
                ...
                for article_element in article_elements:
                    ...
                    if article_response.status_code == 200:
                        ...
                        if article['source'] == "Sky News":
                            article['text'] = self.extract_text_from_website1(article_soup)
                        elif article['source'] == "The Sun":
                            article['text'] = self.extract_text_from_website2(article_soup)
                        elif article['source'] == "BBC":
                            article['text'] = self.extract_text_from_website3(article_soup)
                        elif article['source'] == 'Daily Mail':
                            article['text'] = self.extract_text_from_website4(article_soup)
                        elif article['source'] == 'The Guardian':
                            article['text'] = self.extract_text_from_website5(article_soup)
            ...
            print(f"Failed to retrieve content from {self._url}. Status code: {response.status_code}")
            return []

    def scrape(self):
        return self.extract_articles_from_page(url)
...
web_scraper: DatasetScraper = DatasetScraper(url)
for article in web_scraper.scrape():
    print(article)